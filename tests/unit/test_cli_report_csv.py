...
class DummyScorer:
    ...
    def score_candidate(self, candidate):
        ...
        homepage = candidate.get("homepage") or next(
            (str(u) for u in (candidate.get("urls") or []) if str(u).startswith("http")),
            "",
        )
...
def test_write_report_csv(tmp_path):
    ...
    assert row["documentation_score"] == "0.85"
...
def test_execute_run_emits_csv_with_identifiers(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scorer_module, "Scorer", lambda model=None, config=None: DummyScorer(model))
