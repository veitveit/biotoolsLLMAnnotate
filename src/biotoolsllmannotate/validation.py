...
def validate_config(config: Dict[str, Any]) -> List[str]:
    ...
    # Validate concurrency (now part of Ollama configuration)
    concurrency = ollama_config.get("concurrency", 8)
    if not isinstance(concurrency, int) or concurrency < 1:
        errors.append(
            f"Invalid concurrency value: {concurrency} (must be positive integer)"
        )
    if concurrency > 32:
        errors.append(
            f"Concurrency value {concurrency} may be too high (recommended: ≤32)"
        )
    ...
    for service, service_config in enrichment_config.items():
        if isinstance(service_config, dict) and "timeout" in service_config:
            ...
            if not isinstance(timeout, (int, float)) or timeout <= 0:
                errors.append(
                    f"Invalid timeout for {service}: {timeout} (must be positive number)"
                )
